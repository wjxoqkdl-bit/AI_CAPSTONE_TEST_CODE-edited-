...
## 6. 프롬프트 테스트 방법 (서버 구동 없이)

AI의 키워드 추출 능력을 빠르게 테스트하고 프롬프트를 수정하기 위해, Django 서버를 실행하지 않고 사용할 수 있는 별도의 테스트 스크립트를 제공합니다.

1.  **가상환경 활성화 및 라이브러리 설치** (최초 1회)
    위의 2, 3번 항목을 참고하여 `requirements.txt`의 모든 라이브러리를 설치합니다.
...
3.  **테스트 진행**
    터미널의 안내에 따라 사용자 질문을 입력하면, AI가 추출한 구조화된 키워드(주제, 수식어, 대상)를 바로 확인할 수 있습니다. 프롬프트 수정 시에는 `AICapstone/gptAPI/prompts/` 폴더의 `.json` 파일을 수정하면 됩니다.